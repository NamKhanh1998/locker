...
import { Chain, Circle, Colors, PriceChangePercentage } from '../bubbles.types'
...
export class PixiUtils {
  ...
  static createImageSprite = (circle: Circle, chain: Chain) => {
    const imgUrl = `/tokens/${chain}/${circle.id?.toLowerCase()}.${'png'}`

    const imageSprite = PIXI.Sprite.from(imgUrl)

    const isFullSize = circle.radius * 0.3 <= MIN_FONT_SIZE
